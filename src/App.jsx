...
// ============================================
// NAMENSFILTER - Erweitert mit Russisch, Türkisch & Variationen
// ============================================
const BLOCKED_WORDS = [
  // Deutsch
  ...
  'blyat', 'blya', 'suka', 'suki', 'pizda', 'pizdec', 'pizdeц', 'хуй', 'хуя',
...
// Funktion: Normalisiert Text (ersetzt ähnliche Zeichen, entfernt Sonderzeichen)
function normalizeText(text) {
  if (!text) return '';
  let normalized = text.toLowerCase();
  
  // Leetspeak-Variationen ersetzen (Zahlen → Buchstaben)
  const leetMap = {
  ...
  // Ähnliche Buchstaben ersetzen (für Variationen)
  const similarChars = {
    'ä': 'a', 'ö': 'o', 'ü': 'u', 'ß': 'ss',
    'é': 'e', 'è': 'e', 'ê': 'e', 'ë': 'e',
    'á': 'a', 'à': 'a', 'â': 'a', 'å': 'a',
  ...
  // Doppelte Buchstaben entfernen (z.B. "fuuuck" → "fuck")
...
// Hilfsfunktion: Berechnet Ähnlichkeit zwischen zwei Strings (Levenshtein-Distanz)
...
// Funktion: Prüft ob Name erlaubt ist
...
export default function App() {
  ...
  async function loadData(userId) {
    ...
    try {
      ...
      const { data: gamesData } = await supabase.from('games').select('*').eq('is_cancelled', false).gte('start_time', now.toISOString()).lte('start_time', in7Days.toISOString()).order('start_time', { ascending: true });
      setGames(gamesData || []);
  ...
  async function loadFinishedGames(days) {
    ...
    let query = supabase.from('games').select('*').eq('is_cancelled', false).not('home_score', 'is', null).order('start_time', { ascending: false });
    if (days !== 'all') {
  ...
  async function loadGroups(userId) {
    try {
      ...
      if (memberGroupIds.length > 0) {
        const { data: privateGroups } = await supabase.from('groups').select('*').in('id', memberGroupIds).eq('is_public', false);
  ...
  async function joinGroup() {
    ...
    try {
      const { data: group, error: groupError } = await supabase.from('groups').select('*').eq('join_code', joinCode.toUpperCase()).single();
      if (groupError || !group) { showToast('Ungültiger Beitrittscode.', 'error'); return; }
      const { error: memberError } = await supabase.from('group_members').insert({ group_id: group.id, user_id: user.id, is_admin: false });
      if (memberError) {
  ...
  async function loadGroupDetails(groupId) {
    ...
    const { data: members } = await supabase.from('group_members').select(`*, profiles(username)`).eq('group_id', groupId);
  ...
  async function submitTip(gameId, homeScore, awayScore) {
    ...
    try {
      const { data: existing } = await supabase.from('predictions').select('id').eq('user_id', user.id).eq('game_id', gameId).single();
      if (existing) {
        const { error } = await supabase.from('predictions').update({ predicted_home_score: parseInt(homeScore), predicted_away_score: parseInt(awayScore) }).eq('id', existing.id);
        if (error) throw error;
      } else {
        const { error } = await supabase.from('predictions').insert({ user_id: user.id, game_id: gameId, predicted_home_score: parseInt(homeScore), predicted_away_score: parseInt(awayScore) });
        if (error) throw error;
      }
  ...
  async function deleteTip(gameId) {
    ...
    if (!confirmed) return;
    try {
      const { error } = await supabase.from('predictions').delete().eq('user_id', user.id).eq('game_id', gameId);
      if (error) throw error;
  ...
  function getResultBadge(game) {
    ...
    if (game.home_score === game.away_score) return <span className="badge-draw text-xs px-3 py-1.5 rounded-button font-mono font-bold">UNENTSCHIEDEN</span>;
    ...
    <div className="min-h-screen bg-dark-900">
        ...
        <div className="modal-overlay" onClick={handleConfirmNo}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex gap-3">
              ...
              <button onClick={handleConfirmYes} className="flex-1 glow-button bg-neon-red text-white px-4 py-3 rounded-button font-heading font-semibold hover:bg-red-700 transition">Löschen</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-3">
                ...
                <button onClick={joinGroup} className="flex-1 glow-button bg-gradient-to-r from-neon-gold to-yellow-500 text-dark-900 px-4 py-3 rounded-button font-heading font-bold hover:shadow-glow transition">Beitreten</button>
              </div>
            </div>
          </div>
        </div>
      ...
      <aside className="sidebar hidden lg:flex flex-col">
        ...
        <nav className="flex-1 p-4 space-y-2">
          {[{ id: 'tips', label: 'Spiele', icon: '⚽' }, { id: 'results', label: 'Ergebnisse', icon: '📊' }, { id: 'groups', label: 'Gruppen', icon: '👥' }, { id: 'leaderboard', label: 'Tabelle', icon: '🏆' }].map(tab => (
        ...
        <div className="p-4 border-t border-white/10">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div className="flex-1"><div className="text-sm font-heading font-bold text-white">{username}</div><div className="text-xs text-gray-500 font-body">Online</div></div>
          </div>
          ...
          <div>
            <div className="glass-card rounded-card p-6 mb-6 bg-gradient-to-r from-neon-purple/10 to-neon-pink/10">
              <div className="flex items-center justify-between">
                <div><h1 className="text-2xl md:text-3xl font-heading font-bold text-white mb-2">Hallo {username}! 👋</h1><p className="text-gray-400 font-body">Spiele der nächsten 7 Tage</p></div>
                <div className="text-right hidden md:block"><div className="text-3xl font-heading font-bold text-neon-gold">{leaderboard.find(r => r.username === username)?.total_points || 0}</div><div className="text-sm text-gray-400 font-body">Punkte</div></div>
              </div>
            </div>
                  ...
                  <div key={game.id} ref={el => cardsRef.current[index] = el} className="card-reveal hover-lift glass-card rounded-card overflow-hidden">
                    <div className="p-5">
                      ...
                      <div className="flex justify-between items-center text-base font-heading font-bold mb-4 text-white">
                        <span className="text-right flex-1">{hD}</span><span className="text-neon-gold px-3 text-sm font-normal">vs</span><span className="text-left flex-1">{aD}</span>
                      </div>
                      {started ? (
                        ...
                        <div>
                          <div className="flex items-center gap-2 bg-dark-700 p-3 rounded-button border border-white/5">
                            <input type="number" min="0" placeholder="H" className="w-16 p-2 bg-dark-800 border border-white/10 rounded-button text-center font-mono font-bold text-white focus:border-neon-gold outline-none" value={tips[game.id + 'h'] || ''} onChange={(e) => setTips({ ...tips, [game.id + 'h']: e.target.value })} />
                            <span className="font-bold text-neon-gold">:</span>
                            <input type="number" min="0" placeholder="G" className="w-16 p-2 bg-dark-800 border border-white/10 rounded-button text-center font-mono font-bold text-white focus:border-neon-gold outline-none" value={tips[game.id + 'a'] || ''} onChange={(e) => setTips({ ...tips, [game.id + 'a']: e.target.value })} />
                            <button className="glow-button ml-auto bg-gradient-to-r from-neon-gold to-yellow-500 text-dark-900 px-4 py-2 rounded-button font-heading font-bold hover:shadow-glow transition" onClick={() => submitTip(game.id, tips[game.id + 'h'], tips[game.id + 'a'])}>{hasTip ? 'Ändern' : 'Tippen'}</button>
                          </div>
                ...
                return (
                  <div key={game.id} ref={el => finishedCardsRef.current[index] = el} className="card-reveal hover-lift glass-card rounded-card overflow-hidden">
                    <div className="p-5">
                      ...
                      <div className="text-xs text-gray-500 mb-4 font-body">📅 {dT.toLocaleString('de-DE', { weekday: 'short', day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' })} Uhr</div>
                      <div className="flex justify-between items-center mb-3 text-white"><span className="text-right flex-1 font-heading font-bold">{hD}</span><span className="text-neon-gold font-mono font-bold text-xl px-3">{game.home_score} : {game.away_score}</span><span className="text-left flex-1 font-heading font-bold">{aD}</span></div>
                      {tip && (
                        <div className="mt-3 bg-dark-700 p-3 rounded-button border border-white/5">
                          ...
                          <div className="flex justify-between items-center mt-1"><span className="text-gray-500 text-xs font-body">Punkte:</span><span className={`font-mono font-bold ${points === 5 ? 'text-neon-green' : points === 3 ? 'text-neon-gold' : points === 1 ? 'text-neon-purple' : 'text-neon-red'}`}>{points === 5 ? '🎯 5' : points === 3 ? '👍 3' : points === 1 ? '✓ 1' : '❌ 0'}</span></div>
                        </div>
                      )}
                      {!tip && resultTab === 'all' && <div className="mt-3 text-center text-gray-600 text-xs font-body">Kein Tipp abgegeben</div>}
                    </div>
                  </div>
                );
              })}
          ...
          <div>
            ...
            <div className="glass-card rounded-card p-6 mb-6">
              <div className="flex justify-between items-start mb-4">
                ...
                {selectedGroup.created_by === user.id && <button onClick={() => deleteGroup(selectedGroup.id)} className="glow-button bg-neon-red/20 text-neon-red px-4 py-2 rounded-button font-heading font-semibold hover:bg-neon-red/30 transition">Gruppe löschen</button>}
              </div>
      ...
      <nav className="bottom-nav lg:hidden">
        <div className="flex justify-around items-center">
          {[{ id: 'tips', label: 'Spiele', icon: '⚽' }, { id: 'results', label: 'Ergebnisse', icon: '📊' }, { id: 'groups', label: 'Gruppen', icon: '👥' }, { id: 'leaderboard', label: 'Tabelle', icon: '🏆' }].map(tab => (
            <button key={tab.id} onClick={() => setView(tab.id)} className={`flex flex-col items-center gap-1 px-4 py-2 transition ${view === tab.id ? 'text-neon-gold' : 'text-gray-500 hover:text-gray-300'}`}>
